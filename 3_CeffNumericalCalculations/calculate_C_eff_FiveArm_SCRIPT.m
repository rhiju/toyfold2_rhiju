...
for i = 1:length(sigma)
    C_eff_NAIVE(i) = C_eff_robot_arm_stochastic_NAIVE( sigma(i), NITER, 0.01, 0.01, 5 );
end
figure(2)
...
figure(1)
for i = 1:length(sigma)
    C_eff_NAIVE2(i) = C_eff_robot_arm_stochastic_NAIVE( sigma(i), NITER, 0.003, 0.003, 5 );
end
figure(2)
...
figure(1)
for i = 1:length(sigma)
    C_eff_last_link(i) = C_eff_robot_arm_stochastic_LAST_LINK( sigma(i), NITER, 0.01, 0.01, 5);
end
...
figure(1)
for i = 1:length(sigma)
    C_eff_last_link2(i) = C_eff_robot_arm_stochastic_LAST_LINK( sigma(i), NITER, 0.003, 0.003, 5 );
end
figure(2); clf
...
for i = 1:length(sigma_two_last_links)
    C_eff_two_last_links(i) = C_eff_robot_arm_stochastic_TWO_LAST_LINKS( sigma_two_last_links(i), NITER, 5 );
end
figure(2); clf
...
for i = 1:length(sigma_harmonic);
    C_eff_harmonic( i ) = real( C_eff_SE2_harmonic_expansion( sigma_harmonic(i), 300,10,5 ) );
end
figure(2); clf
loglog( sigma, [C_eff_NAIVE; C_eff_NAIVE2; C_eff_last_link; C_eff_last_link2],'o'); hold on;
...
figure(1)
for i = 1:length(sigma_harmonic);
    C_eff_harmonic2( i ) = real( C_eff_SE2_harmonic_expansion( sigma_harmonic(i),300,20,5 ) );
end
figure(2); clf
loglog( sigma, [C_eff_NAIVE; C_eff_NAIVE2; C_eff_last_link; C_eff_last_link2],'o'); hold on;
...
for i = 1:length(sigma_MM2);
    C_eff_MM2( i ) = C_eff_compute_MM2( sigma_MM2(i), 5 );
end
figure(2); clf
loglog( sigma, [C_eff_NAIVE; C_eff_NAIVE2; C_eff_last_link; C_eff_last_link2],'o'); hold on;
...
%C_eff_semianalytic = [];
figure(1)
sigma_semianalytic = 10.^[-3:0.25:2];
dtheta=0.02;
for i = 1:length(sigma_semianalytic);
    C_eff_semianalytic( i ) = C_eff_compute_semianalytic( sigma_semianalytic(i), 2*pi/5, dtheta );
end
figure(2); clf
...
dL = 0.01; dtheta = 2*pi/100;n_link = 5; theta0 = 2*pi/n_link;
NITER = 10000;
...
C_eff_gaussian_convolution = C_eff_robot_arm_gaussian_convolution(5) ./sigma_fine.^3; 
...
ylabel( 'C_{eff}' );
title( 'C_{eff} predictions for effector with 5 rigid arms to return to origin' );
