...
for i = 1:length(sigma)
    C_eff_NAIVE(i) = C_eff_robot_arm_stochastic_NAIVE( sigma(i), NITER, 0.01, 0.01, n_link );
end
figure(2)
...
figure(1)
for i = 1:length(sigma)
    C_eff_NAIVE2(i) = C_eff_robot_arm_stochastic_NAIVE( sigma(i), NITER, 0.003, 0.003, n_link );
end
figure(2)
...
figure(1)
for i = 1:length(sigma)
    C_eff_last_link(i) = C_eff_robot_arm_stochastic_LAST_LINK( sigma(i), NITER, 0.01, 0.01, n_link);
end
...
figure(1)
for i = 1:length(sigma)
    C_eff_last_link2(i) = C_eff_robot_arm_stochastic_LAST_LINK( sigma(i), NITER, 0.003, 0.003, n_link );
end
figure(2); clf
...
for i = 1:length(sigma_two_last_links)
    C_eff_two_last_links(i) = C_eff_robot_arm_stochastic_TWO_LAST_LINKS( sigma_two_last_links(i), NITER, n_link );
end
figure(2); clf
...
for i = 1:length(sigma_harmonic);
    C_eff_harmonic( i ) = real( C_eff_SE2_harmonic_expansion( sigma_harmonic(i), 300,10,n_link ) );
end
figure(2); clf
loglog( sigma, [C_eff_NAIVE; C_eff_NAIVE2; C_eff_last_link; C_eff_last_link2],'o'); hold on;
...
figure(1)
for i = 1:length(sigma_harmonic);
    C_eff_harmonic2( i ) = real( C_eff_SE2_harmonic_expansion( sigma_harmonic(i),300,20,n_link ) );
end
figure(2); clf
loglog( sigma, [C_eff_NAIVE; C_eff_NAIVE2; C_eff_last_link; C_eff_last_link2],'o'); hold on;
...
for i = 1:length(sigma_MM2);
    C_eff_MM2( i ) = C_eff_compute_MM2( sigma_MM2(i), n_link );
end
figure(2); clf
loglog( sigma, [C_eff_NAIVE; C_eff_NAIVE2; C_eff_last_link; C_eff_last_link2],'o'); hold on;
...
dL = 0.01; dtheta = 2*pi/100; theta0 = 2*pi/n_link;
NITER = 10000;
...
C_eff_gaussian_convolution = C_eff_robot_arm_gaussian_convolution(n_link) ./sigma_fine.^3; 
